...
# 先使用--show-libraries查看那些lib是需要或者可以独立编译的
./bootstrap.sh --show-libraries
./bootstrap.sh --without-libraries=python,graph --prefix=output-arm-gcc-linaro-5.5 
```
生成有 b2 和 bjam、project-config.jam。
修改project-config.jam文件内容
```
...
if ! gcc in [ feature.values <toolset> ]
{
    using gcc : : /media/zhuqingquan/project/code/gcc-linaro-5.5.0-2017.10-x86_64_arm-linux-gnueabihf/bin/arm-linux-gnueabihf-gcc ; 
}
...
```
保存，继续编译。
...
./b2 install 
```
[参考](https://developer.aliyun.com/article/1474138)
...
```
#### 解决arecord无法打开音频设备的权限问题
表现：`arecord -l`无法列出所有的设备，录音也无法打开。
解决方式：
1. 将用户加入到audio用户组 `sudo usermod -aG audio $USER` 。然后注销、重新登录
2. 临时方案。将修改设备文件的读写权限：`sudo chmod a+rw /dev/snd/*`。系统重启后会失效。
